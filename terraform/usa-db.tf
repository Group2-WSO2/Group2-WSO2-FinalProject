resource "google_compute_instance" "db_usa" {
  count = 3
  name  = "db-usa-${count.index}"
  machine_type = "e2-small"
  zone = var.zones_usa[count.index]
  hostname = "${var.app_name}-db-usa${count.index}.${var.app_domain}"
  tags = ["ssh","db"]
  ...
  network_interface {
    ...
    network_ip = "${var.db_ips_usa[count.index]}"
  }
}
